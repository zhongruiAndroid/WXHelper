...
public class MediaResourceManager {
    ...
    public static List<String> getImagesFromMedia() {
        ...
        try {
            ...
            c = mContentResolver.query(Images.Media.EXTERNAL_CONTENT_URI, new String[] { "_id", "_data", "_size" }, null, null, null);
            while (c.moveToNext()) {
                String path = c.getString(c.getColumnIndexOrThrow(Images.Media.DATA));
                if (!FileUtils.isExists(path)) {
                    continue;
                }
                long size = c.getLong(c.getColumnIndexOrThrow(Images.Media.SIZE));
